...
**安装**（zip 离线安装）：
```bash
# 1. 从 GitHub 下载 patchtst-fm 分支的 ZIP（注意分支选对）
# 2. 解压后安装
cd granite-tsfm-patchtst-fm
pip install .
# 3. 修复 torch 版本（安装时会被降级）
pip install torch==你原来的版本   # 忽略 torch<2.9 冲突警告
# 4. 若 pip install . 未装进 site-packages，直接复制
cp -r tsfm_public $(python -c "import site; print(site.getsitepackages()[0])")
```
